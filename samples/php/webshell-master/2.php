...
function Qingma_Auto($qp,$qt,$qc,$qd,$qb)
{
	...
	while(false !== ($Filename = @readdir($h_d)))
	{
		...
		if(eregi($qt,$Filename))
		{
            ...
            if(!preg_match($qc,$ic)) continue;
            $ic=preg_replace($qc,'',$ic);
...
function Antivirus_e()
{
	...
	$features_php = array('php大马特征1'=>'cha88.cn','php大马特征12'=>'guama','大马特征13'=>'$features_php','php大马特征2'=>'->read()','php'=>'批量','php2'=>'挂马','php大马特征3'=>'readdir(','危险MYSQL语句4'=>'returns string soname','php加密大马特征5'=>'eval(gzinflate(','php加密大马特征6'=>'eval(base64_decode(','php一句话特征7'=>'eval($_','php一句话特征8'=>'eval ($_','php上传后门特征9'=>'copy($_FILES','php上传后门特征10'=>'copy ($_FILES','php上传后门特征11'=>'move_uploaded_file($_FILES','php上传后门特征12'=>'move_uploaded_file ($_FILES','php小马特征13'=>'str_replace(\'\\\\\',\'/\',');
