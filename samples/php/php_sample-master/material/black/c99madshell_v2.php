...
if ($act == "sql")
{
 ...
 else
 {
  ...
  if (!empty($sql_db))
  {
   ?><td width="25%" height="100%" valign="top"><a href="#" onclick="document.sql.act.value='sql';document.sql.sql_login.value='<?php echo (htmlspecialchars($sql_login)) ?>';document.sql.sql_passwd.value='<?php echo (htmlspecialchars($sql_passwd)) ?>';document.sql.sql_server.value='<?php echo (htmlspecialchars($sql_server)) ?>';document.sql.sql_port.value='<?php echo (htmlspecialchars($sql_port)) ?>';document.sql.submit();"><b>Home</b></a><hr size="1" noshade><?php
