...
<HTML>
<TITLE>MySQL Web Shell <?php echo $VERSION ?>, <?php echo "$LOGIN@$HOST" ?></TITLE>
...
var lasthist = 0;
var histtouched = <?php echo ($action == "submit")?1:0 ?>;
...
<?php if ($MSIE && $DISABLEDM != "YES"): ?><DIV id="descr" style="position: absolute; top: 25px; left: 0px; visibility: hidden; z-index: 10;"></DIV><?php endif ?>
...
<TD ROWSPAN=2 NOWRAP BGCOLOR="#112255" VALIGN=TOP WIDTH=1%>
 ...
 <TR><TD bgcolor="<?php echo (!$CONNECT && $action == "chparam")?"#660000":"#446688" ?>"><B><?php echo $CONNECT?"Parameters":"Login" ?> <?php if (!$CONNECT && $action == "chparam") echo "failure" ?>:</B></TD></TR>
 <TR><TD><INPUT type=hidden name="action" value="chparam">
 <DIV class="SMALL">
  host:
  <INPUT class="SMALL" size=12 style="width: 120px;" type=text name="host" value="<?php echo $HOST?$HOST:"localhost" ?>"><BR>
  login:
  <INPUT class="SMALL" size=12 style="width: 120px;" type=text name="login" value="<?php echo $LOGIN ?>"><BR>
  password:<BR>
  <INPUT class="SMALL" size=12 style="width: 120px;" type=password name="password"><BR>
  database: <?php if (!$SELECTDB && $DATABASE != "") {echo "<FONT color=\"#FF3333\">access failed</FONT>";} ?>
  <?php if ($CONNECT): ?>                                                                  
  ...
  <?php else: ?>
  <INPUT class="SMALL" size=12 style="width: 120px;" type=text name="database" value="<?php echo $DATABASE ?>">
  <?php endif ?>
  <?php if ($MSIE): ?>
  <BR>
  <B>Drop menu on top:</B><BR>
  <INPUT type=radio name="DISABLEDM" value="YES" <?php if ($DISABLEDM == "YES") echo "checked" ?>> disable
  <INPUT type=radio name="DISABLEDM" value="" <?php if ($DISABLEDM == "") echo "checked" ?>> enable
  <BR><?php endif ?><BR><INPUT class="SMALL" style="width: 120px;" type=submit value="<?php echo $CONNECT?"         submit         ":"         login          " ?>">
 </DIV>
</TD>
</TR>
</FORM>
...
<FORM action="<?php echo $PHP_SELF ?>" method=POST name="queryform">
<TR>
<TD colspan=3>
<TABLE border=0 cellspacing=0 cellpadding=0>
<TR><TD>&nbsp;</TD><TD>Press <B>Ctrl+Enter</B> to submit query<BR></TD></TR>
<TR>
<TD valign=top align=center><BR><A href="javascript: hist('up');"><IMG src="<?php echo $PHP_SELF ?>?arrowup" width=12 height=11 border=0 alt="History Back"></A><BR><BR><A href="javascript: hist('down');"><IMG src="<?php echo $PHP_SELF ?>?arrowdown" width=12 height=11 border=0 alt="History Forward"></A></TD>
<TD>
<INPUT type=hidden name="action" value="submit">
<TEXTAREA name="sqlquery" wrap=off rows=12 cols=59 style="width: 610px; height: 220px; font-family: Verdana; font-size: 11px;"><?php echo htmlspecialchars(stripslashes($sqlquery)) ?></TEXTAREA>
</TD>
</TR></TABLE>
</TD></TR>
<TR>
...
<tr>
 ...
 <td nowrap><input type=radio name="fetchtype" value="1" checked> all rows in one table (<A href="javascript: openWin('<?php echo $PHP_SELF ?>?action=showhelp&number=1');">help</A>)
</tr>
<tr>
 <td nowrap><input type=radio name="fetchtype" value="2"> one row = one table (<A href="javascript: openWin('<?php echo $PHP_SELF ?>?action=showhelp&number=2');">help</A>)
</tr>
</table>
</TD>
...
<?php if ($isfetch): ?><TR bgcolor="#000000"><TD COLSPAN=2 align=center><B>FETCH RESULTS</B></TD></TR><?php endif ?>
...
<?php
  if ($isfetch && $fetchtype == 1)
  {
    ...
    <TABLE border=0 cellspacing=1 cellpadding=2 width=100%><TR bgcolor="#223344"><TD align=center><b><?php 
  ...
  if ($isfetch && $fetchtype == 2)
  {
    ...
    ?>
    <TABLE border=0 cellspacing=1 cellpadding=2 width=100%><TR bgcolor="#223344"><TD width=<?php echo $percent ?>% align=center><b><?php 
    $fields = _mysql_all_fields($qwresult);
...
?>
<HTML>
<TITLE>Fetch Type help : <?php echo ($num==1)?"all rows in one table":"one row = one table" ?></TITLE>
