...
function j_check(source) {
  ...
  function j_jsfile(){
  	var j=prompt('Newfile :<?php echo$getdir; ?>');
  	if(j != null){
  	window.location.href='?j=<?php echo$j_film->Fz($getdir);?>&n='+j+'&a=touch';
    }
  }
  function j_jsdir(){
  	var j=prompt('New Directory : <?php echo$getdir;?>');
  	if(j != null){
  		window.location.href='?j=<?php echo$j_film->Fz($getdir);?>&n='+j+'&a=mkdir';
  	}
  }
</script>
<?php
