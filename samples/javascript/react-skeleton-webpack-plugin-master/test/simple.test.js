...
test('it should insert the ssr result of skeleton into mounted point', async t => {
    ...
    t.true(htmlContent.includes('<div><header class='));
    t.true(htmlContent.includes('<img src="data:image/svg+xml;base64'));

    // inlined css
    t.true(htmlContent.includes('{height:52px;background:blue}'));
});
